...
TEMPLATE_EXPECTATIONS = {
    "conf/cassandra.yaml": (
        'Contains configuration defaults for a "compatible"',
        "Contains configuration defaults that enable",
    ...
    "test/conf/latest_diff.yaml": (
        ...
        "diff conf/cassandra.yaml conf/cassandra_latest.yaml",
    ...
    "test/distributed/org/apache/cassandra/distributed/impl/InstanceConfig.java": (
        "DTEST_JVM_DTESTS_USE_LATEST.getBoolean()",
    ...
    "test/unit/org/apache/cassandra/config/UnitConfigOverride.java": (
        'CDC("cassandra.yaml", "cdc.yaml")',
        'COMPRESSION("cassandra.yaml", "commitlog_compression_LZ4.yaml")',
        ...
        "CONFIG_TYPE is meant to be changed by users while debugging",
        'Files.createTempFile("cassandra-conf", ".yaml")',
        "CassandraRelevantProperties.CASSANDRA_CONFIG.setString(tmp.toURI().toString());",
    ),
    "test/distributed/org/apache/cassandra/distributed/upgrade/ConfigCompatibilityTestGenerate.java": (
        "public class ConfigCompatibilityTestGenerate",
        "This class is to generate YAML dumps per version",
        "for (Semver version : Arrays.asList(v40, v41, v50))",
        'new File(TEST_DIR, "version=" + version + ".yml")',
        "UpgradeableCluster.create(1, latest)",
...
DOC_EXPECTATIONS = {
    ...
    CHECKER_DOC: SCENARIOS + (
        ...
        "compatible/latest distribution templates",
        "17 scenarios",
    ),
    README_DOC: (
        ...
        "compatible/latest 配置模板",
        "`latest_diff.yaml` 与 dtest latest overrides",
        "| Configuration |",
    ),
    SOURCE_MAP_DOC: (
        ...
        "conf/cassandra_latest.yaml:17",
        "test/conf/latest_diff.yaml:19",
        "test/distributed/org/apache/cassandra/distributed/impl/InstanceConfig.java:119",
    ),
}
...
def main(argv):
    ...
    failures.extend(check_tokens(TEMPLATE_EXPECTATIONS, "template"))
