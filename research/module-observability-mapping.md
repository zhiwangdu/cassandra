...
| `snapshots` | snapshot tag、keyspace/table、true size、size on disk、created/expires、ephemeral。 | `system_views.snapshots` 枚举 `StorageService.instance.snapshotManager.loadSnapshots()` 并计算 size，可能触达目录元数据；`listsnapshots` 使用同源 `TableSnapshot` 元数据但经 `SnapshotDetailsTabularData` 格式化成 JMX TabularData。见 `src/java/org/apache/cassandra/db/virtual/SnapshotsTable.java:45-80`、`src/java/org/apache/cassandra/db/SnapshotDetailsTabularData.java:28-83`、`src/java/org/apache/cassandra/tools/nodetool/ListSnapshots.java:56-91` |
