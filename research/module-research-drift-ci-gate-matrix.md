...
| --- | --- | --- | --- |
| `research_checker_inventory_contract` | 当前 research 目录有 73 个 `research/tools/check-*.py` drift checker；新增或删除 checker 时必须同步本矩阵。 | `research/tools/check-research-drift-ci-gate-drift.py:17`, `research/tools/check-testing-ci-generator-drift.py:3`, `research/tools/check-nodetool-runbook-drift.py:3`, `research/tools/check-nodetool-cache-runtime-drift.py:3`, `research/tools/check-nodetool-snapshot-lifecycle-drift.py:3` | 让研究知识库自己的校验入口可枚举，而不是散落在 README 或提交记录里。 |
...
| `research_checker_adjacency_runbook_contract` | 模块文档中提到的相邻 checker 仍按模块运行，例如 testing CI generator、nodetool、nodetool cache runtime、nodetool snapshot lifecycle、JMX compatibility 和 guardrails checker。 | `research/tools/check-testing-ci-generator-drift.py:246`, `research/tools/check-nodetool-runbook-drift.py:135`, `research/tools/check-nodetool-cache-runtime-drift.py:3`, `research/tools/check-nodetool-snapshot-lifecycle-drift.py:3`, `research/tools/check-jmx-compatibility-drift.py:14` | 修改 CI/JMX/nodetool/guardrails 相关源码时，应跑本矩阵 checker 加相邻模块 checker。 |
