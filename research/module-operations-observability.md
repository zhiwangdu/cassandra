...
- `snapshot` 构造 skipFlush/ttl/tag/keyspace.table 参数并经 `StorageService.takeSnapshot()` 进入 `Keyspace.snapshot()` / `ColumnFamilyStore.snapshot()`，写 hard links、manifest/schema 并把 expiring snapshot 交给 `SnapshotManager`；`clearsnapshot` 通过 tag/all/older-than filter 调 `TableSnapshot.shouldClearSnapshot()`，`listsnapshots` 输出 `SnapshotDetailsTabularData`，`setsnapshotthrottle` / `getsnapshotthrottle` 读写 `snapshot_links_per_second`，见 `src/java/org/apache/cassandra/tools/nodetool/Snapshot.java:39-111`、`src/java/org/apache/cassandra/tools/nodetool/ClearSnapshot.java:39-123`、`src/java/org/apache/cassandra/tools/nodetool/ListSnapshots.java:35-91`、`src/java/org/apache/cassandra/tools/nodetool/SetSnapshotThrottle.java:25-34`、`src/java/org/apache/cassandra/service/StorageService.java:4334-4710`、`src/java/org/apache/cassandra/service/snapshot/SnapshotManager.java:48-165`。
...
| `snapshot_before_compaction` / `auto_snapshot` / `auto_snapshot_ttl` / `snapshot_links_per_second` | `src/java/org/apache/cassandra/config/Config.java:312-322`、`conf/cassandra.yaml:1137-1162` | snapshot 创建、安全自动快照、自动快照 TTL 和 hardlink/remove throttle |
